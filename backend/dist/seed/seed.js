...
(async () => {
    ...
    await Promise.all([user_1.User.deleteMany({}), products_1.Product.deleteMany({})]);
    const admin = await user_1.User.create({
        name: "Admin", email: "admin@demo.com", password: "Admin1234", role: "admin",
    });
    await user_1.User.create({ name: "Demo User", email: "user@demo.com", password: "User1234" });
    await products_1.Product.insertMany(mockProducts(admin.id));

 ...
 * Зависимости: react, react-dom, react-router-dom, axios, tailwindcss.
 * main.tsx импортирует <App /> отсюда.
 * Иконки lucide-react заменены на смайлики.
 */
import {
    ...
    <header className="sticky top-0 z-40 bg-white border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 text-royal font-bold text-lg">
          <span aria-hidden="true">{Icon.Building2}</span> Estate
        </Link>
        ...
        <div className="flex items-center gap-3">
          <Link to="/cart" className="relative p-2 text-slate-700 hover:text-royal" aria-label="Корзина">
            <span aria-hidden="true">{Icon.ShoppingCart}</span>
            {items.length > 0 && (
          ...
          {user ? (
            <div className="flex items-center gap-2">
              ...
              <Link to="/profile" className="text-sm text-slate-700 flex items-center gap-1">
                <span aria-hidden="true">{Icon.User}</span> {user.name}
              </Link>
    ...
    <div className="card hover:-translate-y-0.5 transition">
      ...
      <div className="p-4">
        ...
        <p className="text-sm text-slate-500 flex items-center gap-1 mt-1">
          <span aria-hidden="true">{Icon.MapPin}</span> {p.location.city}, {p.location.address}
        </p>
        <div className="mt-3 flex items-center gap-4 text-sm text-slate-600">
          <span className="flex items-center gap-1"><span aria-hidden="true">{Icon.Bed}</span> {p.bedrooms}</span>
          <span className="flex items-center gap-1"><span aria-hidden="true">{Icon.Bath}</span> {p.bathrooms}</span>
          <span className="flex items-center gap-1"><span aria-hidden="true">{Icon.Maximize}</span> {p.area} м²</span>
        </div>
        {user && (
          <button onClick={() => add(p._id)} className="btn-outline w-full mt-3 text-sm">
            <span aria-hidden="true">{Icon.Heart}</span> В избранное
          </button>
        )}
      </div>
    </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-20">
        ...
        <div className="bg-white p-4 rounded-xl shadow-card grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <button className="btn-primary"
            ...
            <span aria-hidden="true">{Icon.Search}</span> Искать
          </button>
        </div>
      </div>
    </section>
...
/* ────────────── СТРАНИЦЫ ────────────── */
const homeCats = [
  { key: "apartment", label: "Квартиры", emoji: Icon.Building2 },
  { key: "house", label: "Дома", emoji: Icon.Home },
  { key: "townhouse", label: "Таунхаусы", emoji: Icon.Warehouse },
  { key: "commercial", label: "Коммерция", emoji: Icon.Store },
];
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2">
        ...
        <div className="flex gap-6 mt-4 text-slate-700">
          <span className="flex items-center gap-1"><span aria-hidden="true">{Icon.Bed}</span> {p.bedrooms} спален</span>
          <span className="flex items-center gap-1"><span aria-hidden="true">{Icon.Bath}</span> {p.bathrooms} с/у</span>
          <span className="flex items-center gap-1"><span aria-hidden="true">{Icon.Maximize}</span> {p.area} м²</span>
        </div>
        ...
        <div className="card p-6">
          ...
          {user && (
            <button className="btn-primary w-full mt-4" onClick={() => add(p._id)}>
              <span aria-hidden="true">{Icon.ShoppingCart}</span> В корзину
            </button>
          )}
        </div>
            ...
            {items.map((p) => (
              <div key={p._id} className="card p-4 flex gap-4">
                ...
                <button onClick={() => remove(p._id)}
                  ...
                  <span aria-hidden="true">{Icon.Trash2}</span>
                </button>
              </div>
            ))}
          </div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {tab === "products" && (
          <Link to="/admin/property/new" className="btn-primary">
            <span aria-hidden="true">{Icon.Plus}</span> Добавить объект
          </Link>
        )}
      </div>
